...
#include "drv_mcp2518fd_defines.h"
#include "drv_mcp2518fd_api.h"

// If device is compiled but not configured, use dummy handlers
// todo, review driver configuration
#ifndef DEVICE_MCP2518FD_SPI_PORT 
#define DEVICE_MCP2518FD_SPI_PORT   0
#endif
#ifndef DEVICE_MCP2518FD_EN 
#define DEVICE_MCP2518FD_EN(x)    
#endif 
#ifndef DEVICE_MCP2518FD_CS 
#define DEVICE_MCP2518FD_CS(x)     
#endif
...
// *****************************************************************************
// Section: Variables
CAN_ERROR_STATS g_canErrStats;
CAN_CONFIGURATION g_canConfig;

uint8_t txBuffer[MCP2518FD_MAX_MSG_SIZE];
uint8_t rxBuffer[MCP2518FD_MAX_MSG_SIZE];
...
int8_t DRV_SPI_ChipSelectAssert(MCP2518_Device device, bool assert)
{
    if (assert)
    {
        // pull down nCS pin (SPI_CAN2_CS, port 1, pin 6 in PCA9673PW), cause nCS pin is low active
        DEVICE_MCP2518FD_CS(0);
    }
    else
    {
        // pull up nCS pin
        DEVICE_MCP2518FD_CS(1);
    }
...
{
    spi_transaction_t tx_t = {0};

    tx_t.cmd = cINSTRUCTION_RESET;
    tx_t.addr = 0;

    ZERO_CHECK(DRV_SPI_ChipSelectAssert(device, true));
    ZERO_CHECK(spi_device_polling_transmit(device, &tx_t));
    ZERO_CHECK(DRV_SPI_ChipSelectAssert(device, false));
...
int8_t DRV_CANFDSPI_ReadByte(MCP2518_Device device, uint16_t address, uint8_t *rxd)
{
    spi_transaction_t rx_t = {0};


    rx_t.flags = SPI_TRANS_USE_RXDATA;
    rx_t.cmd = cINSTRUCTION_READ;
    rx_t.addr = address;
    rx_t.rxlength = sizeof(uint8_t) * 8;

    ZERO_CHECK(DRV_SPI_ChipSelectAssert(device, true));
    ZERO_CHECK(spi_device_polling_transmit(device, &rx_t));
    ZERO_CHECK(DRV_SPI_ChipSelectAssert(device, false));
...
int8_t DRV_CANFDSPI_WriteByte(MCP2518_Device device, uint16_t address, uint8_t txd)
{
    spi_transaction_t tx_t = {0};

    tx_t.flags = SPI_TRANS_USE_TXDATA;
    tx_t.cmd = cINSTRUCTION_WRITE;
    tx_t.addr = address;
    tx_t.length = sizeof(uint8_t) * 8;
    tx_t.tx_data[0] = txd;
...
int8_t DRV_CANFDSPI_ReadWord(MCP2518_Device device, uint16_t address, uint32_t *rxd)
{
    uint8_t i;
    uint32_t x;
    spi_transaction_t rx_t = {0};
    ...
    rx_t.cmd = cINSTRUCTION_READ;
    rx_t.addr = address;
    rx_t.rxlength = sizeof(uint32_t) * 8;
...
int8_t DRV_CANFDSPI_WriteWord(MCP2518_Device device, uint16_t address, uint32_t txd)
{
    uint8_t i;
    spi_transaction_t tx_t = {0};
    ...
    tx_t.cmd = cINSTRUCTION_WRITE;
    tx_t.addr = address;
    tx_t.length = sizeof(uint32_t) * 8;

    for (i = 0; i < 4; i++) {
        tx_t.tx_data[i] = (uint8_t) ((txd >> (i * 8)) & 0xFF);
    }
...
int8_t DRV_CANFDSPI_ReadHalfWord(MCP2518_Device device, uint16_t address, uint16_t *rxd)
{
    uint8_t i;
    uint32_t x;
    spi_transaction_t rx_t = {0};
    ...
    rx_t.cmd = cINSTRUCTION_READ;
    rx_t.addr = address;
    rx_t.rxlength = sizeof(uint16_t) * 8;
...
int8_t DRV_CANFDSPI_WriteHalfWord(MCP2518_Device device, uint16_t address, uint16_t txd)
{
    uint8_t i;
    spi_transaction_t tx_t = {0};
...
int8_t DRV_CANFDSPI_WriteByteSafe(MCP2518_Device device, uint16_t address, uint8_t txd)
{
    uint16_t crcResult = 0;
    uint8_t buffForCrc[3] = {0};
    spi_transaction_t tx_t = {0};

    tx_t.flags = SPI_TRANS_USE_TXDATA;
    tx_t.cmd = cINSTRUCTION_WRITE_SAFE;
    tx_t.addr = address;
    tx_t.length = sizeof(uint8_t) * 3 * 8;


    // Compose command
    buffForCrc[0] = (uint8_t) ((cINSTRUCTION_WRITE_SAFE << 4) + ((address >> 8) & 0xF));
    buffForCrc[1] = (uint8_t) (address & 0xFF);
    buffForCrc[2] = txd;

    // Add CRC
    crcResult = DRV_CANFDSPI_CalculateCRC16(buffForCrc, 3);

    tx_t.tx_data[0] = txd;
    tx_t.tx_data[1] = (crcResult >> 8) & 0xFF;
    tx_t.tx_data[2] = crcResult & 0xFF;
...
int8_t DRV_CANFDSPI_WriteWordSafe(MCP2518_Device device, uint16_t address, uint32_t txd)
{
    uint8_t i;
    uint16_t crcResult = 0;
    uint8_t buffForCrc[8] = {0};
    spi_transaction_t tx_t = {0};

    tx_t.cmd = cINSTRUCTION_WRITE_SAFE;
    tx_t.addr = address;
    tx_t.length = sizeof(uint8_t) * 6 * 8;
    ...
    for (i = 0; i < 4; i++) {
        buffForCrc[i + 2] = (uint8_t) ((txd >> (i * 8)) & 0xFF);
    }

    // Add CRC
    crcResult = DRV_CANFDSPI_CalculateCRC16(buffForCrc, 6);
    buffForCrc[6] = (crcResult >> 8) & 0xFF;
    buffForCrc[7] = crcResult & 0xFF;
    tx_t.tx_buffer = &buffForCrc[2];
...
int8_t DRV_CANFDSPI_ReadByteArray(MCP2518_Device device, uint16_t address, uint8_t *rxd, uint16_t nBytes)
{
    spi_transaction_t rx_t = {0};
...
int8_t DRV_CANFDSPI_WriteByteArray(MCP2518_Device device, uint16_t address, uint8_t *txd, uint16_t nBytes)
{
    spi_transaction_t tx_t = {0};

    tx_t.cmd = cINSTRUCTION_WRITE;
    tx_t.addr = address;
    tx_t.length = nBytes * 8;
    tx_t.tx_buffer = txd;
...
int8_t DRV_CANFDSPI_ReadWordArray(MCP2518_Device device, uint16_t address, uint32_t *rxd, uint16_t nWords)
{
    spi_transaction_t rx_t = {0};

    rx_t.cmd = cINSTRUCTION_READ;
    rx_t.addr = address;
    rx_t.rx_buffer = (uint8_t *)rxd;
    rx_t.rxlength = nWords * sizeof(uint32_t) * 8;
        ...
        uint32_t *txd, uint16_t nWords)
{
    ...
    tx_t.cmd = cINSTRUCTION_WRITE;
    tx_t.addr = address;
    tx_t.length = nWords * sizeof(uint32_t) * 8;
    tx_t.tx_buffer = (uint8_t *)txd;
...
int8_t DRV_CANFDSPI_Configure(MCP2518_Device device, CAN_CONFIG* config)
{
    ...
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiCON, ciCon.word);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_OPERATION_MODE opMode)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_CiCON + 3, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_CiCON + 3, d);
    if (spiTransferError) {
        return -2;
    }
...
CAN_OPERATION_MODE DRV_CANFDSPI_OperationModeGet(MCP2518_Device device)
{
    ...
    // Read Opmode
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_CiCON + 2, &d);
    if (spiTransferError) {
...
int8_t DRV_CANFDSPI_LowPowerModeEnable(MCP2518_Device device)
{
...
#else
    // Read
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_OSC, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_OSC, d);
    if (spiTransferError) {
        return -2;
    }
#endif
        ...
        CAN_FIFO_CHANNEL channel, CAN_TX_FIFO_CONFIG* config)
{
    ...
    //calculate CAN message address
    a = cREGADDR_CiFIFOCON + (channel * CiFIFO_OFFSET);

    spiTransferError = DRV_CANFDSPI_WriteWord(device, a, ciFifoCon.word);
        ...
        uint8_t *txd, uint32_t txdNumBytes, bool flush)
{
    ...
    REG_CiFIFOUA ciFifoUa = {0};
    int8_t spiTransferError = 0;

    // Get FIFO registers
    a = cREGADDR_CiFIFOCON + (channel * CiFIFO_OFFSET);

    spiTransferError = DRV_CANFDSPI_ReadWordArray(device, a, fifoReg, 3);
    if (spiTransferError) {
        return -1;
    }
    ...
    memset(txBuffer, 0, MCP2518FD_MAX_MSG_SIZE);

    memcpy(txBuffer, txObj->byte, MCP2518FD_TX_RX_OBJ_HDR_BYTES); 
    memcpy(&txBuffer[8], txd, txdNumBytes);
    ...
    if (txdNumBytes % 4) {
        // Need to add bytes
        n = 4 - (txdNumBytes % 4);
        i = txdNumBytes + MCP2518FD_TX_RX_OBJ_HDR_BYTES;
    ...
    spiTransferError = DRV_CANFDSPI_WriteByteArray(device, a, txBuffer, txdNumBytes + MCP2518FD_TX_RX_OBJ_HDR_BYTES + n);
    if (spiTransferError) {
        return -4;
    }

    // Set UINC and TXREQ
    spiTransferError = DRV_CANFDSPI_TransmitChannelUpdate(device, channel, flush);
    if (spiTransferError) {
        return -5;
    }
        ...
        CAN_FIFO_CHANNEL channel)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, d);
        ...
        CAN_FIFO_CHANNEL channel, CAN_TX_FIFO_STATUS* status)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWordArray(device, a, fifoReg, 2);
    if (spiTransferError) {
        return -1;
    }
...
int8_t DRV_CANFDSPI_TransmitChannelReset(MCP2518_Device device,
        CAN_FIFO_CHANNEL channel)
{
    return DRV_CANFDSPI_ReceiveChannelReset(device, channel);
}
        ...
        CAN_FIFO_CHANNEL channel, bool flush)
{
    ...
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, ciFifoCon.byte[1]);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_TXREQ_CHANNEL txreq)
{
    ...
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiTXREQ, w);
        ...
        uint32_t* txreq)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_CiTXREQ, txreq);
        ...
        CAN_FILTER filter, CAN_FILTEROBJ_ID* id)
{
    ...
    fObj.word = 0;
    fObj.bF = *id;
    a = cREGADDR_CiFLTOBJ + (filter * CiFILTER_OFFSET);  // Get address

    spiTransferError = DRV_CANFDSPI_WriteWord(device, a, fObj.word);
        ...
        CAN_FILTER filter, CAN_MASKOBJ_ID* mask)
{
    ...
    mObj.word = 0;
    mObj.bF = *mask;
    a = cREGADDR_CiMASK + (filter * CiFILTER_OFFSET);  // Get address

    spiTransferError = DRV_CANFDSPI_WriteWord(device, a, mObj.word);
        ...
        CAN_FILTER filter, CAN_FIFO_CHANNEL channel, bool enable)
{
    ...
    a = cREGADDR_CiFLTCON + filter;  // Get address

    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, fCtrl.byte);
...
int8_t DRV_CANFDSPI_FilterEnable(MCP2518_Device device, CAN_FILTER filter)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &fCtrl.byte);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_DNET_FILTER_SIZE dnfc)
{
    ...
    // Read CiCON byte 0
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_CiCON, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_CiCON, d);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_FIFO_CHANNEL channel, CAN_RX_FIFO_CONFIG* config)
{
    ...
    a = cREGADDR_CiFIFOCON + (channel * CiFIFO_OFFSET);  // Get address
        ...
        CAN_FIFO_CHANNEL channel, CAN_RX_FIFO_STATUS* status)
{
    ...
    // Read
    ciFifoSta.word = 0;
    a = cREGADDR_CiFIFOSTA + (channel * CiFIFO_OFFSET);  // Get address

    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &ciFifoSta.byte[0]);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_FIFO_CHANNEL channel, uint8_t *rxd, uint8_t nBytes)
{
    ...
    // Get FIFO registers
    addr = cREGADDR_CiFIFOCON + (channel * CiFIFO_OFFSET);

    spiTransferError = DRV_CANFDSPI_ReadWordArray(device, addr, fifoReg, 3);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Get Status
    ciFifoSta.word = fifoReg[1];
    if (!ciFifoSta.rxBF.RxNotEmptyIF)
    {
        return -5;
    }

    if (ciFifoSta.rxBF.RxOverFlowIF)
    {
        g_canErrStats.overflow_counter++;
        ciFifoSta.rxBF.RxOverFlowIF = 0;
        DRV_CANFDSPI_WriteWord(device, addr + 1, ciFifoSta.word);
    }
    else if (ciFifoSta.rxBF.RxFullIF)
    {
        g_canErrStats.full_counter++;
    }
    else
    {
        if (g_canErrStats.overflow_counter)
        {
            g_canErrStats.overflow_counter--;
        }
        
        if (g_canErrStats.full_counter)
        {
            g_canErrStats.full_counter--;
        }
    }
    ...
    ciFifoUa.word = fifoReg[2];
#ifdef USERADDRESS_TIMES_FOUR
    addr = 4 * ciFifoUa.bF.UserAddress;
#else
    addr = ciFifoUa.bF.UserAddress;
#endif
    addr += cRAMADDR_START;

    spiTransferError = DRV_CANFDSPI_ReadByteArray(device, addr, rxd, nBytes);
    if (spiTransferError) {
        return -3;
    }

    // UINC channel
    spiTransferError = DRV_CANFDSPI_ReceiveChannelUpdate(device, channel);
    if (spiTransferError) {
        return -4;
    }
        ...
        CAN_TEF_FIFO_STATUS* status)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &ciTefSta.byte[0]);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_TEF_MSGOBJ* tefObj)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByteArray(device, a, ba, n);
    if (spiTransferError) {
        return -2;
    }
    ...
    // Set UINC
    spiTransferError = DRV_CANFDSPI_TefUpdate(device);
    if (spiTransferError) {
        return -3;
    }
...
int8_t DRV_CANFDSPI_TefReset(MCP2518_Device device)
{
    ...
    // Write byte
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, ciTefCon.byte[1]);
...
int8_t DRV_CANFDSPI_TefConfigure(MCP2518_Device device, CAN_TEF_CONFIG* config)
{
    ...
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiTEFCON, ciTefCon.word);
        ...
        CAN_MODULE_EVENT* flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadHalfWord(device, cREGADDR_CiINTFLAG, &intFlags.word);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_MODULE_EVENT flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadHalfWord(device, a, &intEnables.word);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteHalfWord(device, a, intEnables.word);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_MODULE_EVENT flags)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteHalfWord(device, a, intFlags.word);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_RXCODE* rxCode)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &rxCodeByte);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_TXCODE* txCode)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &txCodeByte);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_FILTER* filterHit)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &filterHitByte);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_ICODE* icode)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &icodeByte);
    if (spiTransferError) {
        return -1;
    }
...
int8_t DRV_CANFDSPI_TransmitEventGet(MCP2518_Device device, uint32_t* txif)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_CiTXIF, txif);
        ...
        uint32_t* txatif)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_CiTXATIF, txatif);
        ...
        CAN_FIFO_CHANNEL channel, uint8_t* idx)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWord(device, a, &ciFifoSta.word);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_FIFO_CHANNEL channel, CAN_TX_FIFO_EVENT flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &ciFifoCon.byte[0]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, ciFifoCon.byte[0]);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_FIFO_CHANNEL channel)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, ciFifoSta.byte[0]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t DRV_CANFDSPI_ReceiveEventGet(MCP2518_Device device, uint32_t* rxif)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_CiRXIF, rxif);

    return spiTransferError;
}

int8_t DRV_CANFDSPI_ReceiveEventOverflowGet(MCP2518_Device device,
        uint32_t* rxovif)
{
    int8_t spiTransferError = 0;

    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_CiRXOVIF, rxovif);
...
int8_t DRV_CANFDSPI_ReceiveChannelIndexGet(MCP2518_Device device,
        CAN_FIFO_CHANNEL channel, uint8_t* idx)
{
    return DRV_CANFDSPI_TransmitChannelIndexGet(device, channel, idx);
}
        ...
        CAN_TEF_FIFO_EVENT flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &ciTefCon.byte[0]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, ciTefCon.byte[0]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t DRV_CANFDSPI_TefEventOverflowClear(MCP2518_Device device)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, ciTefSta.byte[0]);
    if (spiTransferError) {
        return -2;
    }
        ...
        uint8_t* tec)
{
    ...
    // Read Error count
    a = cREGADDR_CiTREC + 1;  // Get address

    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, tec);
        ...
        uint8_t* rec)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, rec);
        ...
        CAN_ERROR_STATE* flags)
{
    ...
    // Read Error state
    a = cREGADDR_CiTREC + 2;  // Get address
    uint8_t f = 0;

    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &f);
    if (spiTransferError) {
        return -1;
    }
        ...
        uint8_t* tec, uint8_t* rec, CAN_ERROR_STATE* flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWord(device, a, &ciTrec.word);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_BUS_DIAGNOSTIC* bd)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadWordArray(device, a, w, 2);
    if (spiTransferError) {
        return -1;
    }
...
int8_t DRV_CANFDSPI_BusDiagnosticsClear(MCP2518_Device device)
{
    ...
    spiTransferError = DRV_CANFDSPI_WriteWordArray(device, a, w, 2);
...
int8_t DRV_CANFDSPI_EccEnable(MCP2518_Device device)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_ECCCON, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_ECCCON, d);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_ECC_EVENT* flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &eccStatus);
    if (spiTransferError) {
        return -1;
    }
        ...
        uint8_t parity)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_ECCCON + 1, parity);
        ...
        uint8_t* parity)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_ECCCON + 1, parity);
        ...
        uint16_t* a)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_ECCSTA, &reg.word);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_ECC_EVENT flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &eccInterrupts);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, eccInterrupts);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_ECC_EVENT flags)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &eccStat);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, eccStat);
    if (spiTransferError) {
        return -2;
    }
        ...
        CAN_CRC_EVENT flags)
{
    ...
    a = cREGADDR_CRC + 3;  // Get address
    uint8_t crc;

    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &crc);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, crc);
    if (spiTransferError) {
        return -2;
    }
...
int8_t DRV_CANFDSPI_CrcValueGet(MCP2518_Device device, uint16_t* crc)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadHalfWord(device, cREGADDR_CRC, crc);
...
int8_t DRV_CANFDSPI_RamInit(MCP2518_Device device, uint8_t d)
{
    ...
    for (k = 0; k < (cRAM_SIZE / SPI_DEFAULT_BUFFER_LENGTH); k++) {
        spiTransferError = DRV_CANFDSPI_WriteByteArray(device, a, txd, SPI_DEFAULT_BUFFER_LENGTH);
        if (spiTransferError) {
            return -1;
        }
...
int8_t DRV_CANFDSPI_TimeStampEnable(MCP2518_Device device)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_CiTSCON + 2, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_CiTSCON + 2, d);
    if (spiTransferError) {
        return -2;
    }
...
int8_t DRV_CANFDSPI_TimeStampGet(MCP2518_Device device, uint32_t* ts)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadWord(device, cREGADDR_CiTBC, ts);
...
int8_t DRV_CANFDSPI_TimeStampSet(MCP2518_Device device, uint32_t ts)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiTBC, ts);
        ...
        uint16_t ps)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteHalfWord(device, cREGADDR_CiTSCON, ps);
        ...
        CAN_OSC_CTRL ctrl)
{
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, cREGADDR_OSC, osc.byte[0]);
        ...
        CAN_OSC_STATUS* status)
{
    ...
    // Read
    spiTransferError = DRV_CANFDSPI_ReadByte(device, cREGADDR_OSC + 1, &osc.byte[1]);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_SYSCLK_SPEED clk)
{
    ...
    switch (clk) {
        case CAN_SYSCLK_40M:
            spiTransferError = DRV_CANFDSPI_BitTimeConfigureNominal40MHz(device, bitTime);
            if (spiTransferError) return spiTransferError;

            spiTransferError = DRV_CANFDSPI_BitTimeConfigureData40MHz(device, bitTime, sspMode);
            break;
        case CAN_SYSCLK_20M:
            spiTransferError = DRV_CANFDSPI_BitTimeConfigureNominal20MHz(device, bitTime);
            if (spiTransferError) return spiTransferError;

            spiTransferError = DRV_CANFDSPI_BitTimeConfigureData20MHz(device, bitTime, sspMode);
            break;
        case CAN_SYSCLK_10M:
            spiTransferError = DRV_CANFDSPI_BitTimeConfigureNominal10MHz(device, bitTime);
            if (spiTransferError) return spiTransferError;

            spiTransferError = DRV_CANFDSPI_BitTimeConfigureData10MHz(device, bitTime, sspMode);
            break;
        default:
            spiTransferError = -1;
            break;
        ...
        CAN_BITTIME_SETUP bitTime)
{
    ...
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiNBTCFG, ciNbtcfg.word);
        ...
        CAN_BITTIME_SETUP bitTime, CAN_SSP_MODE sspMode)
{
    ...
    // Write Bit time registers
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiDBTCFG, ciDbtcfg.word);
    if (spiTransferError) {
        return -2;
    }
    ...
    spiTransferError = DRV_CANFDSPI_WriteWord(device, cREGADDR_CiTDC, ciTdc.word);
    if (spiTransferError) {
        return -3;
    }
        ...
        GPIO_PIN_MODE gpio0, GPIO_PIN_MODE gpio1)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &iocon.byte[3]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, iocon.byte[3]);
    if (spiTransferError) {
        return -2;
    }
        ...
        GPIO_PIN_DIRECTION gpio0, GPIO_PIN_DIRECTION gpio1)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &iocon.byte[0]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, iocon.byte[0]);
    if (spiTransferError) {
        return -2;
    }
        ...
        GPIO_OPEN_DRAIN_MODE mode)
{
    ...
    // Read
    a = cREGADDR_IOCON + 3;  // Get address
    REG_IOCON iocon;
    iocon.word = 0;
        ...
        GPIO_PIN_POS pos, GPIO_PIN_STATE latch)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &iocon.byte[1]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = DRV_CANFDSPI_WriteByte(device, a, iocon.byte[1]);
    if (spiTransferError) {
        return -2;
    }
        ...
        GPIO_PIN_POS pos, GPIO_PIN_STATE* state)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &iocon.byte[2]);
    if (spiTransferError) {
        return -1;
    }
        ...
        CAN_FIFO_CHANNEL channel, uint8_t* mi)
{
    ...
    spiTransferError = DRV_CANFDSPI_ReadByte(device, a, &b);
    if (spiTransferError) {
        return -1;
    }
...
/*****************************************************************************
 * \name   DRV_MCP2518FD_Init
 * \brief  Initialize MCP2518FD
 * \return Error code (success = ESP_OK)
 *****************************************************************************/
int8_t DRV_MCP2518FD_Init(MCP2518_Device device, CAN_TX_CONFIG txCfg, CAN_RX_CONFIG rxCfg, CAN_FILTER_MASK_CONFIG fltMaskCfg, CAN_TIMING_CONFIG timeCfg)
{
    ...
    memset(&g_canErrStats, 0, sizeof(CAN_ERROR_STATS));
    memset(&g_canConfig, 0, sizeof(CAN_CONFIGURATION));

    // Reset device
    DRV_CANFDSPI_Reset(device);

    // Enable ECC and initialize RAM
    DRV_CANFDSPI_EccEnable(device);

    DRV_CANFDSPI_RamInit(device, 0xff);


    // Configure device
    DRV_CANFDSPI_ConfigureObjectReset(&config);

    DRV_CANFDSPI_Configure(device, &config);

    // Setup TX FIFO
    DRV_CANFDSPI_TransmitChannelConfigureObjectReset(&txConfig);
    txConfig.FifoSize = txCfg.fifo_size;
    txConfig.PayLoadSize = txCfg.payload_size;
    txConfig.TxPriority = txCfg.tx_priority;

    DRV_CANFDSPI_TransmitChannelConfigure(device, txCfg.can_tx_fifo, &txConfig);

    // Setup RX FIFO
    DRV_CANFDSPI_ReceiveChannelConfigureObjectReset(&rxConfig);
    rxConfig.FifoSize = rxCfg.fifo_size;
    rxConfig.PayLoadSize = rxCfg.payload_size;

    DRV_CANFDSPI_ReceiveChannelConfigure(device, rxCfg.can_rx_fifo, &rxConfig);


    // Setup RX Filter
    fObj.word = 0;
    fObj.bF.SID = fltMaskCfg.SID;
    fObj.bF.EXIDE = fltMaskCfg.EXIDE;
    fObj.bF.EID = fltMaskCfg.EID;
    fObj.bF.SID11 = fltMaskCfg.SID11;
    DRV_CANFDSPI_FilterObjectConfigure(device, fltMaskCfg.filterId, &fObj.bF);

    // Setup RX Mask
    mObj.word = 0;
    mObj.bF.MSID = fltMaskCfg.MSID;
    mObj.bF.MEID = fltMaskCfg.MEID;
    mObj.bF.MSID11 = fltMaskCfg.MSID11;
    mObj.bF.MIDE = fltMaskCfg.MIDE;
    DRV_CANFDSPI_FilterMaskConfigure(device, fltMaskCfg.filterId, &mObj.bF);

    // Link FIFO and Filter
    DRV_CANFDSPI_FilterToFifoLink(device, fltMaskCfg.filterId, rxCfg.can_rx_fifo, true);

    // Setup Bit Time
    DRV_CANFDSPI_BitTimeConfigure(device, timeCfg.bit_time, timeCfg.ssp_mode, timeCfg.sys_clk);

    // Setup Transmit and Receive Interrupts
    DRV_CANFDSPI_GpioModeConfigure(device, GPIO_MODE_INT, GPIO_MODE_INT);
    DRV_CANFDSPI_TransmitChannelEventEnable(device, txCfg.can_tx_fifo, CAN_TX_FIFO_NOT_FULL_EVENT);
    DRV_CANFDSPI_ReceiveChannelEventEnable(device, rxCfg.can_rx_fifo, CAN_RX_FIFO_NOT_EMPTY_EVENT);
    DRV_CANFDSPI_ModuleEventEnable(device, CAN_TX_EVENT | CAN_RX_EVENT);

    // Select Normal Mode
    DRV_CANFDSPI_OperationModeSelect(device, CAN_NORMAL_MODE);

    memcpy(&g_canConfig.txCfg, &txCfg, sizeof(CAN_TX_CONFIG));
    memcpy(&g_canConfig.rxCfg, &rxCfg, sizeof(CAN_RX_CONFIG));
    memcpy(&g_canConfig.fltMaskCfg, &fltMaskCfg, sizeof(CAN_FILTER_MASK_CONFIG));
    memcpy(&g_canConfig.timeCfg, &timeCfg, sizeof(CAN_TIMING_CONFIG));

    return 0;
}

/*****************************************************************************
 * \name   DRV_MCP2518FD_Write
 * \brief  send message to MCP2518FD
 * \return Error code (success = ESP_OK)
 *****************************************************************************/
int8_t DRV_MCP2518FD_Write(    
    uint32_t u32Id,     // in: CAN message ID
    bool   bExt,      // in: CAN ID format (0=Standard, 1=Extended)
    uint8_t  u8Dlc,     // in: Length of the CAN message (in bytes)
    uint8_t  *au8Data  // in: Pointer to data array to be sent
    )
{
    CAN_TX_MSGOBJ txObj = {0};
    uint8_t txData[MCP2518FD_MAX_DATA_BYTES] = {0};
    uint8_t txDataLen = 0;
    uint8_t len = (u8Dlc < MCP2518FD_MAX_DATA_BYTES) ? u8Dlc : MCP2518FD_MAX_DATA_BYTES;
    int8_t ret = 0;
    ...
    {
        /* Extended format */
        txObj.bF.id.SID = ((u32Id >> 18) & 0x7FF);
        txObj.bF.id.EID = (u32Id & 0x3FFFF);

    }
    else
    {
        /* Standard format */
        txObj.bF.id.SID = (u32Id & 0x7FF);
    }

    txObj.bF.ctrl.DLC = DRV_CANFDSPI_DataBytesToDlc(u8Dlc);
    txObj.bF.ctrl.IDE = bExt;  // base format 

    
    txDataLen = DRV_CANFDSPI_DlcToDataBytes(txObj.bF.ctrl.DLC);
    memcpy(txData, au8Data, len);

    ret = DRV_CANFDSPI_TransmitMessage(spi3, g_canConfig.txCfg.can_tx_fifo, &txObj, txData, txDataLen, true);

    return ret;
}

/*****************************************************************************
 * \name   DRV_MCP2518FD_Read
 * \brief  receive message from MCP2518FD
 * \return Error code (success = ESP_OK)
 *****************************************************************************/
int8_t DRV_MCP2518FD_Read(    
    uint32_t *u32Id,   // out: ID of received CAN message 
    bool   *bExt,    // out: Format of received CAN message (0=Standard, 1=Extended)
    uint8_t  *u8Dlc,   // out: Data length of received CAN message (in bytes).
    uint8_t  *au8Data // out: Data array of received CAN message
)
{
    CAN_RX_MSGOBJ rxObj = {0};
    uint8_t rxd[MCP2518FD_MAX_DATA_BYTES] = {0};
    int8_t ret = 0;


    ret = DRV_CANFDSPI_ReceiveMessage(spi3, g_canConfig.rxCfg.can_rx_fifo, rxd, MCP2518FD_TX_RX_OBJ_HDR_BYTES + MCP2518FD_CAN_2_0_MAX_BYTES);
    
    if (!ret)
    {
        memcpy(rxObj.byte, rxd, 8);

        if (rxObj.bF.ctrl.IDE & 0x1)
        {
            /* extended format */
            *u32Id = (rxObj.bF.id.SID << 18);
            *u32Id |= rxObj.bF.id.EID;
        }
        else
        {
            /* standard format */
            *u32Id = (uint32_t)rxObj.bF.id.SID;
        }

        *bExt = (rxObj.bF.ctrl.IDE & 0x1);
        *u8Dlc = (uint8_t)rxObj.bF.ctrl.DLC;
        memcpy(au8Data, rxd + 8, rxObj.bF.ctrl.DLC);
