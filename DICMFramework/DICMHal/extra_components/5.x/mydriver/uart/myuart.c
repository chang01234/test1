...
#include "driver/myuart_select.h"
...
esp_err_t uart_isr_register(uart_port_t uart_num, void (*fn)(void *), void *arg, int intr_alloc_flags,  uart_isr_handle_t *handle)
{
    ...
    if (ret == ESP_OK) {
        ...
        if (handle != NULL) {
            *handle = (uart_isr_handle_t)rethandle;
        }
    }
    return ret;
}
...
esp_err_t uart_flush(uart_port_t uart_num) __attribute__((alias("myuart_flush_input")));
