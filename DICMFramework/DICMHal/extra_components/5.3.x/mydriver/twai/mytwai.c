...
#if CONFIG_PM_ENABLE
    //Acquire PM lock
    twai_obj_t *p_twai_obj = g_twai_objs[0];
    if (p_twai_obj && p_twai_obj->pm_lock) 
    {
        ESP_ERROR_CHECK(esp_pm_lock_acquire(p_twai_obj->pm_lock));     //Acquire pm_lock during the whole driver lifetime
    }
#endif //CONFIG_PM_ENABLE
...
void get_print_twai_bus_error(void)
{
    ...
    twai_obj_t *p_obj = g_twai_objs[0];
    twai_ll_parse_err_code_cap(p_obj->hal.dev, &type, &dir, &seg);    //Decode error interrupt
   /*
   ...
   */
    ...
    ESP_EARLY_LOGI(TWAI_TAG, "twai errc fault = %d, dir fault = %d, seg fault = %d", type, dir, seg);
#endif
}