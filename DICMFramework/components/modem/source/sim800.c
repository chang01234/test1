 ...
 */
static esp_err_t sim800_handle_creg_status(modem_dce_t *dce, const char *line)
{
    esp_err_t err = ESP_FAIL;
    sim800_modem_dce_t *sim800_dce = __containerof(dce, sim800_modem_dce_t, parent);
    if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
        ...
        /* store value of result_code and status */
        uint32_t **cbc = sim800_dce->priv_resource;
        /* +CREG: <result_code>,<status> */
        sscanf(line, "%*s%d,%d", cbc[0], cbc[1]);
        err = ESP_OK;
    }
    return err;
}
...
static esp_err_t sim800_handle_atd_ppp(modem_dce_t *dce, const char *line)
{
    ...
    if (strstr(line, MODEM_RESULT_CODE_CONNECT)) {
        ...
        ESP_LOGI(DCE_TAG, "ATD failed: NO CARRIER");
        err = esp_modem_process_command_done(dce, MODEM_STATE_FAIL);
    }
    return err;
}
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t sim800_get_netw_reg_status(modem_dce_t *dce, uint32_t *result_code, uint32_t *status)
{
    modem_dte_t *dte = dce->dte;
    sim800_modem_dce_t *sim800_dce = __containerof(dce, sim800_modem_dce_t, parent);
    uint32_t *resource[2] = {result_code, status};
    sim800_dce->priv_resource = resource;
    dce->handle_line = sim800_handle_creg_status;
...
static esp_err_t sim800_set_working_mode(modem_dce_t *dce, modem_mode_t mode)
{
    ...
    case MODEM_COMMAND_MODE:
        ...
        // Could already be in data mode at this point if the modem itself disconnected.
        if (dte->send_cmd(dte, "+++", MODEM_COMMAND_TIMEOUT_MODE_CHANGE) != ESP_OK)
            ESP_LOGW(DCE_TAG, "enter command mode timed out");
...
/**
 * @brief Set unsolicited network registration status
 *
 * @param sim800_dce sim800 object
 * @param enable Enable status reports
 *      - 1 to enable
 *      - 0 to disable
 * @return esp_err_t
 *      - ESP_OK on success
 *      - ESP_FAIL on error
 */
static esp_err_t sim800_set_network_registration_status(sim800_modem_dce_t *sim800_dce, int enable)
{
    modem_dte_t *dte = sim800_dce->parent.dte;
    sim800_dce->parent.handle_line = sim800_handle_creg;
    if (enable) {
        DCE_CHECK(dte->send_cmd(dte, "AT+CREG=1\r", MODEM_COMMAND_TIMEOUT_OPERATOR) == ESP_OK, "send command failed", err);
    } else {
        DCE_CHECK(dte->send_cmd(dte, "AT+CREG=0\r", MODEM_COMMAND_TIMEOUT_OPERATOR) == ESP_OK, "send command failed", err);
    }
    DCE_CHECK(sim800_dce->parent.state == MODEM_STATE_SUCCESS, "set network registration status failed", err);
    ESP_LOGD(DCE_TAG, "get network operator ok");
    return ESP_OK;
err:
    return ESP_FAIL;
