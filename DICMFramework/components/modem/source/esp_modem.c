...
#ifndef CONFIG_MODEM_UART_TX_PIN
#if defined(CONFIG_DICM_TARGET_SDB)			//MODEM UART RX/TX pin configured for SDB hardware
#define CONFIG_MODEM_UART_TX_PIN MODEM_TXD_PIN
#else
#define CONFIG_MODEM_UART_TX_PIN 25
#endif
#endif
#ifndef CONFIG_MODEM_UART_RX_PIN
#if defined(CONFIG_DICM_TARGET_SDB)			//MODEM UART RX/TX pin configured for SDB hardware
#define CONFIG_MODEM_UART_RX_PIN MODEM_RXD_PIN
#else
#define CONFIG_MODEM_UART_RX_PIN 35
#endif
#endif

#ifndef CONFIG_MODEM_UART_RTS_PIN
#define CONFIG_MODEM_UART_RTS_PIN 27  // Dont care
#endif
#ifndef CONFIG_MODEM_UART_CTS_PIN
#define CONFIG_MODEM_UART_CTS_PIN 23  // Dont care
#endif
#define CONFIG_MODEM_UART_EVENT_TASK_STACK_SIZE 3072
#define CONFIG_MODEM_UART_EVENT_TASK_PRIORITY 5
#define CONFIG_MODEM_UART_EVENT_QUEUE_SIZE 30
#define CONFIG_MODEM_UART_PATTERN_QUEUE_SIZE 60
#define CONFIG_MODEM_UART_TX_BUFFER_SIZE 512
#define CONFIG_MODEM_UART_RX_BUFFER_SIZE 6144
    ...
    while (1) {
        if (xQueueReceive(esp_dte->event_queue, &event, pdMS_TO_TICKS(100))) {
            ...
            case UART_BREAK:
                ...
                // Modem sends break signal when connection has been terminated.
                esp_event_post_to(esp_dte->event_loop_hdl, ESP_MODEM_EVENT, MODEM_EVENT_PPP_DISCONNECT, NULL, 0, pdMS_TO_TICKS(100));
                break;
...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
        res = uart_set_pin(esp_dte->uart_port, CONFIG_MODEM_UART_TX_PIN, CONFIG_MODEM_UART_RX_PIN,
                           CONFIG_MODEM_UART_RTS_PIN, CONFIG_MODEM_UART_CTS_PIN);
    } else {
        res = uart_set_pin(esp_dte->uart_port, CONFIG_MODEM_UART_TX_PIN, CONFIG_MODEM_UART_RX_PIN,
...
static void on_ppp_status_changed(ppp_pcb *pcb, int err_code, void *ctx)
{
    ...
    case PPPERR_NONE: /* Connected */
        ipinfo.ip = pppif->ip_addr;
        ipinfo.gw = pppif->gw;
        ipinfo.netmask = pppif->netmask;
    ...
    case PPPERR_CONNECT: /* Connection lost */
        ESP_LOGE(MODEM_TAG, "Connection lost");
        break;
...
static uint32_t pppos_low_level_output(ppp_pcb *pcb, uint8_t *data, uint32_t len, void *ctx)
{
    ...
    // Do not send data if modem is in command mode.
    if ((dte->dce == NULL) || (dte->dce->mode == MODEM_COMMAND_MODE)) {
        return len;
    }
...
esp_err_t esp_modem_setup_ppp(modem_dte_t *dte)
{
    ...
    MODEM_CHECK(dce, "DTE has not yet bind with DCE", err);
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
    /* Hang up any active connections. */
...
#error "Unsupported AUTH Negotiation"
    ...
    else
    {
        ...
        MODEM_CHECK(pppapi_close(esp_dte->ppp, 1) == ERR_OK, "close ppp connection failed", err); // TODO: Hangs here!
    }
...
esp_err_t esp_modem_change_to_cmd_mode(modem_dte_t *dte)
{
    modem_dce_t *dce = dte->dce;
    MODEM_CHECK(dce, "DTE has not yet bind with DCE", err);
    //esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
