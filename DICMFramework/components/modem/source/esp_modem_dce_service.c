...
esp_err_t esp_modem_dce_sw_reset(modem_dce_t *dce, uint32_t enable)
{
    modem_dte_t *dte = dce->dte;
    DCE_CHECK(dte, "DTE not binded to DCE", err);
    dce->handle_line = esp_modem_dce_handle_response_default;
    if (enable)
        DCE_CHECK(dte->send_cmd(dte, "AT+CFUN=1\r", MODEM_COMMAND_TIMEOUT_DEFAULT*5) == ESP_OK, "send CFUN=1 failed", err);
    else
        DCE_CHECK(dte->send_cmd(dte, "AT+CFUN=0\r", MODEM_COMMAND_TIMEOUT_DEFAULT*5) == ESP_OK, "send CFUN=0 failed", err);
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "CFUN failed", err);
    ESP_LOGD(DCE_TAG, "CFUN ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}

esp_err_t esp_modem_dce_deregister(modem_dce_t *dce)
{
    modem_dte_t *dte = dce->dte;
    DCE_CHECK(dte, "DTE not binded to DCE", err);
    dce->handle_line = esp_modem_dce_handle_response_default;
    DCE_CHECK(dte->send_cmd(dte, "AT+CFUN=4\r", MODEM_COMMAND_TIMEOUT_DEFAULT*5) == ESP_OK, "send CFUN=4 failed", err);
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "CFUN=4 failed", err);
    ESP_LOGD(DCE_TAG, "CFUN=4 ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
