...
//-----------------------------------------------------------------------------
// Function:    PROPDGN_DGN_61184_Extract
//-----------------------------------------------------------------------------
// Description: Extract parameters from DGN 61184
//-----------------------------------------------------------------------------
// Return:      None.
//-----------------------------------------------------------------------------
void PROPDGN_DGN_61184_Extract(
    PROPDGN_zDGN_61184 *pzDest,  // out: Destination structure
    uint8 *pu8Src,               // in:  Source buffer
    uint8 size)
{
    memcpy(pzDest->u8Data, pu8Src, size);
}

//-----------------------------------------------------------------------------
// Function:    PROPDGN_DGN_61184_Stuff
//-----------------------------------------------------------------------------
// Description: Stuff parameters in DGN 61184
//-----------------------------------------------------------------------------
// Return:      None.
//-----------------------------------------------------------------------------
void PROPDGN_DGN_61184_Stuff(uint8 *pu8Dest, PROPDGN_zDGN_61184 *pzSrc)
{
    // Clear
    (void)memset(pu8Dest, 0, PROPDGN_DGN_61184_SIZE);

    size_t data_size = pzSrc->size;

    memcpy(pu8Dest, pzSrc->u8Data, data_size);
    // Pass size as last byte
    pu8Dest[PROPDGN_DGN_61184_SIZE - 1] = data_size;
    // Pass destination address as next last byte
    pu8Dest[PROPDGN_DGN_61184_SIZE - 2] = pzSrc->addr;
}

//-----------------------------------------------------------------------------
// Function:    PROPDGN_DGN_Oper42_Extract
//-----------------------------------------------------------------------------
// Description: Extract parameters from DGN
//-----------------------------------------------------------------------------
// Return:      None.
//-----------------------------------------------------------------------------
void PROPDGN_DGN_Oper42_Extract(
    PROPDGN_zDGN_Oper42 *pzDest,  // out: Destination structure
    uint8 *pu8Src                 // in:  Source buffer
)
{
    //           dest                                src           start size
    DGN_TO_BITS(pzDest->u8Operation, pu8Src[0], 0, 8);
    DGN_TO_BITS(pzDest->u2Purifier, pu8Src[1], 0, 2);
    DGN_TO_BITS(pzDest->u2RemCtrl, pu8Src[1], 2, 2);
    DGN_TO_BITS(pzDest->u2ActExt, pu8Src[1], 4, 2);
    DGN_TO_BITS(pzDest->u2HtrOn, pu8Src[1], 6, 2);
    DGN_TO_BITS(pzDest->u2CompOn, pu8Src[2], 0, 2);
    DGN_TO_BITS(pzDest->u2HFAvl, pu8Src[2], 2, 2);
    DGN_TO_BITS(pzDest->u2LFAvl, pu8Src[2], 4, 2);
}

//-----------------------------------------------------------------------------
// Function:    PROPDGN_DGN_Oper42_Stuff
//-----------------------------------------------------------------------------
// Description: Stuff parameters from DGN
//-----------------------------------------------------------------------------
// Return:      None.
//-----------------------------------------------------------------------------
void PROPDGN_DGN_Oper42_Stuff(
    uint8 *pu8Dest,             // out:  Destination buffer
    PROPDGN_zDGN_Oper42 *pzSrc  // in:   Source structure
)
{
    // Clear
    (void)memset(pu8Dest, 0, PROPDGN_DGN_61184_Oper0x42_SIZE);
    //           dest       source                                     start size
    BITS_TO_DGN(pu8Dest[0], pzSrc->u8Operation, 0, 8);
    BITS_TO_DGN(pu8Dest[1], pzSrc->u2Purifier, 0, 2);
    BITS_TO_DGN(pu8Dest[1], pzSrc->u2RemCtrl, 2, 2);
    BITS_TO_DGN(pu8Dest[1], pzSrc->u2ActExt, 4, 2);
    BITS_TO_DGN(pu8Dest[1], pzSrc->u2HtrOn, 6, 2);
    BITS_TO_DGN(pu8Dest[2], pzSrc->u2CompOn, 0, 2);
    BITS_TO_DGN(pu8Dest[2], pzSrc->u2HFAvl, 2, 2);
    BITS_TO_DGN(pu8Dest[2], pzSrc->u2LFAvl, 4, 2);
}
