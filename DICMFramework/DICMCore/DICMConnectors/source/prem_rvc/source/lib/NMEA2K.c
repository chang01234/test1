...
{
    uint8 IdentityNumber_Byte1;
    uint8 IdentityNumber_Byte2;
    struct
    {
        bit IdentityNumber_Byte3 : 5;
...
static nmea2k_RxMultiPacket_State_Variables_Struct_Type nmea2k_RxMultiPacket[NMEA2K_INSTANCE_COUNT][NMEA2K_NUMBER_RX_MULTI_PACKET_SESSION];
static nmea2k_TxMultiPacket_State_Variables_Struct_Type nmea2k_TxMultiPacket[NMEA2K_INSTANCE_COUNT][NMEA2K_NUMBER_TX_MULTI_PACKET_SESSION];
#endif

// Use to keep track of the Multi Packet session states and variables
#if NMEA2K_SUPPORT_FAST_PACKET_RX
static nmea2k_RxFastPacket_State_Variables_Struct_Type RxFastPacket[NMEA2K_INSTANCE_COUNT][NMEA2K_NUMBER_RX_FAST_PACKET_SESSION];
#endif
...
static uint32 Nmea2k_RxManagerTask(void);
...
static uint8 Nmea2k_Dll_GetRandomTimeDelay(uint8 *NameFld);
...
extern void MEA2K_SetAddressClaimedCallback(address_claimed_callback_t address_claimed_cb)
{
...
//-----------------------------------------------------------------------------
uint8 MEA2K_GetSourceAddr(uint8 u8CanPort)
{
...
)
{
    ...
    {
        hal_can_resume(u8CanPort, false);
    }
...
)
{
    ...
    PGN_Requested.IdField.BitField.UnusedMSB = 0;
    PGN_Requested.Identifier = (uint32_t)TPCMBAM.PduFormat | (uint32_t)(TPCMBAM.PduSpecific << 8);
    ...
    if ((nmea2k_RxMultiPacket_PTR != NULL) && (Session_Abort == 0))
    {
        ...
        // Try to find a VALID Free J1939 Rx Mailbox for the new Message.
        nmea2k_RxMultiPacket_PTR->RxMsgPtr = Nmea2k_DLL_Check_nmea2kRxMailbox_MultiPacket(PGN_Requested.Identifier, Originator_Address);
...
static uint8 Nmea2k_Tx_DataTransfert(nmea2k_TxMultiPacket_State_Variables_Struct_Type *TxMultiPacket_PTR)  // Pointer to the current MultiPacket State structure to build the message
{
    ...
    if (Nmea2k_Dll_TxSingleFrameMessage(&message) == TRUE)  // Send the Data Transfert Message
    {
        ...
        if (TxMultiPacket_PTR->DA == 255)
        {
            ...
            TxMultiPacket_PTR->DelayTimer = 150;                                        // Tx Time Out set at 1500mS
...
//-----------------------------------------------------------------------------
static uint8 Nmea2k_Dll_GetRandomTimeDelay(
...
static uint32 Nmea2k_RxManagerTask(void)
{
    ...
    for (dsr = 0; dsr < UINT8_MAX; dsr++)
    {
        ...
        if ((id.IdField.PduFormat <= NMEA2K_MAX_PF_PDU1FORMAT) && ((id.IdField.PduSpecific == nmea2k_MyAddress[nmea2k_u8Instance]) || (id.IdField.PduSpecific == NMEA2K_GLOBAL_ADDRESS)))
        {
