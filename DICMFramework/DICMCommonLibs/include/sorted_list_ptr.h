...
@{
Description for sorted_list_ptr.h
========================
...
- In this implementation, the key and value are both of type uintptr_t.
...
Example:
    ...
    DECLARE_SORTED_LIST_PTR(unique, 16);
    DECLARE_SORTED_LIST_PTR(single, 16);
    DECLARE_SORTED_LIST_PTR(multiple, 16);

    int main(void)
    {
        sorted_list_ptr_unique_add(&unique, 1, 10);
        sorted_list_ptr_unique_add(&unique, 1, 10);
        sorted_list_ptr_unique_add(&unique, 1, 30);
        sorted_list_ptr_unique_add(&unique, 2, 10);
        sorted_list_ptr_unique_add(&unique, 2, 30);

        sorted_list_ptr_single_add(&single, 1, 10);
        sorted_list_ptr_single_add(&single, 1, 10);
        sorted_list_ptr_single_add(&single, 1, 30);
        sorted_list_ptr_single_add(&single, 2, 10);
        sorted_list_ptr_single_add(&single, 2, 30);

        sorted_list_ptr_multiple_add(&multiple, 1, 10);
        sorted_list_ptr_multiple_add(&multiple, 1, 10);
        sorted_list_ptr_multiple_add(&multiple, 1, 30);
        sorted_list_ptr_multiple_add(&multiple, 2, 10);
        sorted_list_ptr_multiple_add(&multiple, 2, 30);
...
Version history:
    ...
    2022-05-13: Added SORTED_LIST_PTR_NO_CHANGE to sorted_list_ptr_remove_value()
    ...
    2024-10-08: Added sorted_list_ptr_remove_entries()
    2025-05-06: Added uintptr_t variant
    2025-10-23: Reformatting

@} */

#ifndef SORTED_LIST_PTR_H
#define SORTED_LIST_PTR_H
...
//! \~ Declare a private SORTED_LIST_PTR with private data storage
#define DECLARE_SORTED_LIST_PTR(name, size) \
    static SORTED_LIST_PTR_ENTRY name##_data[size]; \
    static SORTED_LIST_PTR name = { .pdata = name##_data, .capacity = size, .entry_count = 0, }

//! \~ Declare a public SORTED_LIST_PTR with public data storage
#define DECLARE_SORTED_LIST_PTR_PUBLIC(name, size) \
    SORTED_LIST_PTR_ENTRY name##_data[size]; \
    SORTED_LIST_PTR name = { .pdata = name##_data, .capacity = size, .entry_count = 0, }

//! \~ Declare a private SORTED_LIST_PTR with private data storage in EXTRAM
#define DECLARE_SORTED_LIST_PTR_EXTRAM(name, size) \
    static EXT_RAM_ATTR SORTED_LIST_PTR_ENTRY name##_data[size]; \
    static SORTED_LIST_PTR name = { .pdata = name##_data, .capacity = size, .entry_count = 0, }

//! @brief Declare a private SORTED_LIST_PTR with private data storage in EXTRAM.
//!
//! @note need to allocate buffer on heap using INIT_SORTED_LIST_EXTRAM_PTR
#define DECLARE_SORTED_LIST_PTR_EXTRAM_PTR(name, size) \
    static EXT_RAM_ATTR SORTED_LIST_PTR_ENTRY *name##_data; \
    static SORTED_LIST_PTR name = { .pdata = NULL, .capacity = size, .entry_count = 0, }

#define INIT_SORTED_LIST_PTR_EXTRAM_PTR(name) \
    name##_data = hal_mem_malloc_prefer(sizeof(SORTED_LIST_PTR_ENTRY) * name.capacity, HAL_MEM_SPIRAM, HAL_MEM_INTERNAL_RAM); \
    assert(name##_data != NULL); \
    name.pdata = name##_data

//! \~ Declare a public SORTED_LIST_PTR with public data storage in EXTRAM
#define DECLARE_SORTED_LIST_PTR_EXTRAM_PUBLIC(name, size)                             \
    EXT_RAM_ATTR SORTED_LIST_PTR_ENTRY name##_data[size]; \
    SORTED_LIST_PTR name = { .pdata = name##_data, .capacity = size, .entry_count = 0, }

typedef uintptr_t SORTED_LIST_PTR_KEY_TYPE;    //!< \~	Key data type (uintptr_t)
typedef uintptr_t SORTED_LIST_PTR_VALUE_TYPE;  //!< \~	Value data type (uintptr_t)

//! \~ Sorted list entry; key+value
typedef struct SORTED_LIST_PTR_ENTRY
{
    SORTED_LIST_PTR_KEY_TYPE key;      //!< \~ Subscribed parameter number
    SORTED_LIST_PTR_VALUE_TYPE value;  //!< \~ Subscribed connector or device on connector
} SORTED_LIST_PTR_ENTRY;

//! \~ Sorted list metadata
typedef struct SORTED_LIST_PTR
{
    SORTED_LIST_PTR_ENTRY *pdata;  //!< \~ Pointer to sorted list data
    int capacity;                      //!< \~ Capacity of sorted list in entries
    int entry_count;                   //!< \~ Number of entries currently in list
} SORTED_LIST_PTR;

//! \~ Function return values
typedef enum SORTED_LIST_PTR_RETURN_VALUE
{
    SORTED_LIST_PTR_FAIL = 0,                        //!< \~
    SORTED_LIST_PTR_ENTRY_UPDATED = 1,               //!< \~
    SORTED_LIST_PTR_ENTRY_INSERTED = 2,              //!< \~
    SORTED_LIST_PTR_OK = 3,                          //!< \~
    SORTED_LIST_PTR_COMPARE_RESULT_GREATER = 4,      //!< \~ Entry comparison result >
    SORTED_LIST_PTR_COMPARE_RESULT_EQUAL = 5,        //!< \~ Entry comparison result <=
    SORTED_LIST_PTR_COMPARE_RESULT_LESS = 6,         //!< \~ Entry comparison result <
    SORTED_LIST_PTR_NO_CHANGE = 7,                   //!< \~ List was not updated
    SORTED_LIST_PTR_INVALID_ARGUMENT = 8,            //!< \~ An argument was invalid
} SORTED_LIST_PTR_RETURN_VALUE;
...
SORTED_LIST_PTR_RETURN_VALUE sorted_list_ptr_clear(SORTED_LIST_PTR *const sortlist);
