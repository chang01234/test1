...
#define ARGUMENT_CHECK(x) TRUE_CHECK_RETURNX(SORTED_LIST64_INVALID_ARGUMENT, (x))
...
/*! \brief Compare an entry in a sorted list to a candidate entry
    ...
    \retval SORTED_LIST64_COMPARE_RESULT_GREATER Entry comparison result >
    \retval SORTED_LIST64_COMPARE_RESULT_EQUAL Entry comparison result <=
    \retval SORTED_LIST64_COMPARE_RESULT_LESS Entry comparison result <
 */
static SORTED_LIST64_RETURN_VALUE sorted_list64_compare_entries(const SORTED_LIST64_ENTRY *const search_entry, const SORTED_LIST64_KEY_TYPE insert_key, const SORTED_LIST64_VALUE_TYPE insert_value)
{
    ...
    if (greater_than)
    {
        return SORTED_LIST64_COMPARE_RESULT_GREATER;  // false in both cases
    }

    if (equal)
    {
        return SORTED_LIST64_COMPARE_RESULT_EQUAL;  // true in rightmost case
    }

    return SORTED_LIST64_COMPARE_RESULT_LESS;  // true in both cases
}
 ...
 */
static int sorted_list64_entry_search(int *const position, const SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value)
{
    ...
    {
        imid = (imin + imax) >> 1;
        if (sorted_list64_compare_entries(&sorted_list->pdata[imid], key, value) == SORTED_LIST64_COMPARE_RESULT_LESS)  // leftmost search (<)
        {
            imin = imid + 1;
        }
        else
...
/*! \brief Inserts a new entry into a sorted list
    ...
    \retval SORTED_LIST64_FAIL Sorted list was full
    \retval SORTED_LIST64_ENTRY_INSERTED Entry was inserted
 */
static SORTED_LIST64_RETURN_VALUE sorted_list64_insert_entry(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value, const int position)
{
    ...
    {
        return SORTED_LIST64_FAIL;
    }

    const size_t Move_size = sizeof(SORTED_LIST64_ENTRY) * (sorted_list->entry_count - (size_t)position);
    ...
    return SORTED_LIST64_ENTRY_INSERTED;
}
...
/*! \brief Overwrites an old entry in sorted list
    ...
    \retval SORTED_LIST64_ENTRY_UPDATED Entry was updated
    \retval SORTED_LIST64_NO_CHANGE Entry already existed
 */
static SORTED_LIST64_RETURN_VALUE sorted_list64_overwrite_entry(const SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value, const int position)
{
    ARGUMENT_CHECK(position >= 0);

    SORTED_LIST64_VALUE_TYPE old_value = sorted_list->pdata[position].value;

    sorted_list->pdata[position].key = key;
    sorted_list->pdata[position].value = value;

    return (value == old_value) ? SORTED_LIST64_NO_CHANGE : SORTED_LIST64_ENTRY_UPDATED;
}
...
{
    SORTED_LIST64_ENTRY *sorted_list_entry;
    SORTED_LIST64 *sorted_list;
    size_t sorted_list_entry_size = sizeof(SORTED_LIST64_ENTRY) * count;
...
#ifdef IDF_VER
    sorted_list = hal_mem_malloc_prefer(sizeof(SORTED_LIST64), HAL_MEM_SPIRAM, HAL_MEM_INTERNAL_RAM);
#else
    sorted_list = malloc(sizeof(SORTED_LIST64));
#endif  // IDF_VER
    if (sorted_list == NULL)
    {
#ifdef IDF_VER
...
{
    SORTED_LIST64_ENTRY *sorted_list_entry = sorted_list->pdata;
...
/*! \brief Remove an entry at position in list
    ...
    \retval SORTED_LIST64_OK Entry was removed
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_remove_entry(SORTED_LIST64 *const sorted_list, const int position)
{
    ...
    const size_t Move_size = sizeof(SORTED_LIST64_ENTRY) * (sorted_list->entry_count - (size_t)position - 1ull);

    memmove(&sorted_list->pdata[position], &sorted_list->pdata[position + 1], Move_size);

    sorted_list->entry_count--;

    return SORTED_LIST64_OK;
}

/*! \brief Deletes all elements in list
    \param sorted_list Sorted list to clear
    \retval SORTED_LIST64_OK List was cleared
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_clear(SORTED_LIST64 *const sorted_list)
{
    sorted_list->entry_count = 0;
...
/*! \brief Get all keys containing value in list
    ...
    \retval SORTED_LIST64_OK Key/value pair was found in list, keys assigned
    \retval SORTED_LIST64_FAIL Value was not found in list, keys not assigned
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_get_keys(SORTED_LIST64_KEY_TYPE *const keys, int *key_count, SORTED_LIST64 *const sorted_list, const SORTED_LIST64_VALUE_TYPE value, const int remove)
{
    ...
    for (int position = 0; position < (int)sorted_list->entry_count; position++)
    {
        if ((count < capacity) && (position < sorted_list->entry_count) && (sorted_list->pdata[position].value == value))
        {
            ...
            if (remove)
            {
                sorted_list64_remove_entry(sorted_list, position--);
            }
        }
    }

    *key_count = count;
    return count ? SORTED_LIST64_OK : SORTED_LIST64_FAIL;
}

/*! \brief Remove all entries containing value
    \param sorted_list Sorted list to modify
    \param value Value to remove from list
    \retval SORTED_LIST64_OK At least one entry was removed
    \retval SORTED_LIST64_NO_CHANGE No entries contained value
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_remove_value(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_VALUE_TYPE value)
{
    ...
    return item_was_removed ? SORTED_LIST64_OK : SORTED_LIST64_NO_CHANGE;
}
...
SORTED_LIST64_RETURN_VALUE sorted_list64_remove_entries(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value)
{
    ...
    if (sorted_list64_entry_search(&position, sorted_list, key, value))
    {
...
/*! \brief Get value for key in an unique list
    ...
    \retval SORTED_LIST64_OK Key/value pair was found in list, value assigned
    \retval SORTED_LIST64_FAIL Key was not found in list, value not assigned
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_unique_get(SORTED_LIST64_VALUE_TYPE *const value, SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const int remove)
{
    ...
    if (sorted_list64_key_search(&position, sorted_list, key))
    {
...
/*! \brief Remove an entry from a unique list
    ...
    \retval SORTED_LIST64_OK Key was found in list and entry was deleted
    \retval SORTED_LIST64_NO_CHANGE Key was not found in list, entry not deleted
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_unique_remove(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key)
{
    ...
    if (sorted_list64_key_search(&position, sorted_list, key))
    {
        return sorted_list64_remove_entry(sorted_list, position);  // if entry exists, remove it
    }
    else
    {
        return SORTED_LIST64_NO_CHANGE;
    }
}
...
/*! \brief Add an entry to a unique list
    ...
    \retval SORTED_LIST64_ENTRY_UPDATED Key was found in list and entry was updated
    \retval SORTED_LIST64_NO_CHANGE Key already existed with supplied value
    \retval SORTED_LIST64_ENTRY_INSERTED Key was not found in list, entry was inserted
    \retval SORTED_LIST64_FAIL Sorted list was full, entry could not be inserted
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_unique_add(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value)
{
    ...
    if (sorted_list64_key_search(&position, sorted_list, key))
    {
        return sorted_list64_overwrite_entry(sorted_list, key, value, position);  // if entry already exists, overwrite it
    }
    else
    {
        return sorted_list64_insert_entry(sorted_list, key, value, position);  // otherwise insert it
    }
}
...
/*! \brief Get value for key in a single list
    ...
    \retval SORTED_LIST64_OK Key/value pair was found in list, value assigned
    \retval SORTED_LIST64_FAIL Key was not found in list, values not assigned
 */
...
/*! \brief Remove entries from a single list
    \param sorted_list Sorted list to search
    \param key Key to remove
    \retval SORTED_LIST64_OK Key was found in list and at least one entry was deleted
    \retval SORTED_LIST64_NO_CHANGE Key was not found in list, nothing was deleted
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_single_remove(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key)
{
    ...
    int position;
    SORTED_LIST64_RETURN_VALUE result = SORTED_LIST64_NO_CHANGE;

    if (sorted_list64_key_search(&position, sorted_list, key))
    {
        result = SORTED_LIST64_OK;
...
/*! \brief Add an entry to a single list
    ...
    \param value Value of entry to add
    \retval SORTED_LIST64_NO_CHANGE Key already existed with supplied value
    \retval SORTED_LIST64_ENTRY_INSERTED Key/value pair was not found in list, entry was inserted
    \retval SORTED_LIST64_FAIL Sorted list was full, entry could not be inserted
 */
SORTED_LIST64_RETURN_VALUE sorted_list64_single_add(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value)
{
    ...
    if (sorted_list64_entry_search(&position, sorted_list, key, value))
    {
        return SORTED_LIST64_NO_CHANGE;  // if entry already exists, do nothing
    }
    else
    {
        return sorted_list64_insert_entry(sorted_list, key, value, position);  // otherwise insert it
...
{
    return sorted_list64_single_get(values, value_count, sorted_list, key, remove);
}
...
{
    return sorted_list64_single_remove(sorted_list, key);
}
...
SORTED_LIST64_RETURN_VALUE sorted_list64_multiple_add(SORTED_LIST64 *const sorted_list, const SORTED_LIST64_KEY_TYPE key, const SORTED_LIST64_VALUE_TYPE value)
{
    ...
    sorted_list64_entry_search(&position, sorted_list, key, value);

    return sorted_list64_insert_entry(sorted_list, key, value, position);
}
...
/*! \brief Count how many entries match key/value pair in multiple list
    ...
    \param remove True: Remove entry from list if key is found
    \retval SORTED_LIST64_OK Key/value pair was found in list, count assigned
    \retval SORTED_LIST64_FAIL Key was not found in list, count not assigned
 */
