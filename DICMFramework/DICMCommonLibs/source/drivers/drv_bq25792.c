 ...
 * Local defines
 *****************************************************************************/
#define DRV_BQ25792_DEBUG 0
static bq25792_dev bq25792_dev_inst;
...
#if DRV_BQ25792_DEBUG
bq25792_reg_addr reg_arr[] =
{
...
/*! \brief Initialize BQ25792 LCD Driver IC
...
error_type bq25792_init(const drv_bus_conf* const bus_conf)
{
    ...
    /* Set the device ID */
    bq25792_dev_inst.dev_id = bus_conf->i2c.port;
    /* Set the slave address */
    bq25792_dev_inst.slave_address = DEV_PMIC_BQ25792_I2C_SLAVE_ADDR;

    /* Wait for 1000 msec before accessing BQ25792 I2C registers.. ( Recommended as per TI datasheeet is 25 msec ) */
    osal_task_delay(1000);

    /* Read the BQ25792 IC part number */
    ...
    if ( RES_PASS == result )
    {
        ...
        {
            bq25792_dev_inst.chip_id = BQ25792_PART_NUMBER;
        }
        LOG(W, "BQ25792 PART_NUMBER : %d chip_id : %d", bqic_part_info.PART_NUMBER, bq25792_dev_inst.chip_id );

        /* Reset the watchdog timer */
        result = bq25792_wd_reset();

        /* Battery cell configuartion read */
        result = bq25792_read_reg(RECHARGE_CONTROL_REG0AH, &rechg_ctrl_reg.byte, ONE_BYTE);
        LOG(W, "Before config RECHARGE_CONTROL_REG0AH = 0x%x result = %d", rechg_ctrl_reg.byte, result);

        rechg_ctrl_reg.CELL = THREE_CELL;
        result = bq25792_write_reg(RECHARGE_CONTROL_REG0AH, &rechg_ctrl_reg.byte, ONE_BYTE);
        ...
        #ifdef EMC_TEST_CASE
           result = bq25792_set_charging_current_limit(CHARGE_CURRENT_200MA);    /* Set charging current for EMC testing*/     
        #else
        ...
        /* send the data to bq25792 */
        result = bq25792_write_reg(MINIMUM_SYSTEM_VOLTAGE_REG0H, &byte, ONE_BYTE);

        byte = 0;
        result = bq25792_read_reg(MINIMUM_SYSTEM_VOLTAGE_REG0H, &byte, ONE_BYTE);
        LOG(W, "MINIMUM_SYSTEM_VOLTAGE_REG0H = 0x%x result = %d", byte, result);

        byte2 = SWAP2(CHARGE_VOLTAGE_LIMIT_VALUE);
        /* Send the data to bq25792 */
        result = bq25792_write_reg(CHARGE_VOLTAGE_LIMIT_REG01H, (uint8_t*)&byte2, TWO_BYTE);

        byte2 = 0;
        result = bq25792_read_reg(CHARGE_VOLTAGE_LIMIT_REG01H, (uint8_t*)&byte2, TWO_BYTE);
        LOG(W, "CHARGE_VOLTAGE_LIMIT_REG01H = 0x%x result = %d", SWAP2(byte2), result);

        byte2 = 0;
        result = bq25792_read_reg(CHARGE_CURRENT_LIMIT_REG03H, (uint8_t*)&byte2, TWO_BYTE);
        LOG(W, "CHARGE_CURRENT_LIMIT_REG03H = 0x%x result = %d", SWAP2(byte2), result);


        result = bq25792_read_reg(TIMER_CONTROL_REG0EH, (uint8_t*)&timer_ctrl_reg.byte, ONE_BYTE);
        LOG(W, "Before config TIMER_CONTROL_REG0EH = 0x%x result = %d", timer_ctrl_reg.byte, result);

        //!Set TSHUT  cut off temperature
        result = bq25792_read_reg(TEMPERATURE_CONTROL_REG16H, (uint8_t*)&byte2, ONE_BYTE);
        byte2 = byte2 & (~BQ25792_REG18_TS_COOL_BIT_CLEAR);
        byte2 = byte2 & (~BQ25792_REG18_TS_WARM_BIT_CLEAR);

        LOG(W, "Before TEMP_CTRL_REG18H = 0x%x result = %d", byte2, result);
        byte2 = byte2 | BQ25792_REG18_TS_COOL_71_1 ; 
        byte2 = byte2 | BQ25792_REG18_TS_WARM_37_7;
        result = bq25792_write_reg(NTC_CONTROL_1_REG18H, (uint8_t*)&byte2, ONE_BYTE);
        
        //!Set TSHUT  cut off temperature
        result = bq25792_read_reg(TEMPERATURE_CONTROL_REG16H, (uint8_t*)&byte2, ONE_BYTE);
        LOG(W, "Before config TEMPERATURE_CONTROL_REG16H = 0x%x result = %d", byte2, result);
        byte2 = byte2 & (~BQ25792_REG16_TSHUT_BIT_CLEAR);
        byte2 = byte2 & (~BQ25792_REG16_TREG_BIT_CLEAR);

        byte2 = byte2 | BQ25792_REG16_TSHUT_85C ; //set Thut cut off to 85 degree C value = 0x30;
        byte2 = byte2 | BQ25792_REG16_TREG_65C;
        result = bq25792_write_reg(TEMPERATURE_CONTROL_REG16H, (uint8_t*)&byte2, ONE_BYTE);

        timer_ctrl_reg.CHG_TMR = TWELVE_HOURS;
        /* send the data to bq25792 */
        result = bq25792_write_reg(TIMER_CONTROL_REG0EH, &timer_ctrl_reg.byte, ONE_BYTE);
        LOG(W, "After config TIMER_CONTROL_REG0EH = 0x%x result = %d", timer_ctrl_reg.byte, result);

        byte = INPUT_VOLTAGE_LIMIT_VALUE;
        result = bq25792_write_reg(INPUT_VOLTAGE_LIMIT_REG05H, &byte, ONE_BYTE);

        byte = 0;

        result = bq25792_read_reg(INPUT_VOLTAGE_LIMIT_REG05H, (uint8_t*)&byte, ONE_BYTE);
        LOG(W, "Before config INPUT_VOLTAGE_LIMIT_REG05H = 0x%x result = %d", byte, result);

        /* Start ADC conversion */
        result = bq25792_start_adc_conversion();
        
        LOG(W, "BATTERY IC BQ25792 INIT Success = %d", result);
    }
    else
    {
        LOG(E, "BATTERY IC BQ25792 INIT FAILED error = %d", result);
    }
...
{
    return bq25792_dev_inst.chip_id;
}
...
error_type bq25792_set_charging_current_limit(uint16_t chg_current)
{
    ...
    if ( ichg != data2 )
    {
        ...
        result = bq25792_write_reg(CHARGE_CURRENT_LIMIT_REG03H, (uint8_t*)&data2, 2u);
    }
    else
    {
...
/*! \brief  This function used to reset the internal watch dog timer of the battery charger IC BQ25792 before it get expires
 *  \param  void
 *  \return result 0 - Pass / -1 - Fail
 */
error_type bq25792_wd_reset(void)
{
    ...
    /* send the data to bq25792 */
    result = bq25792_write_reg(CHARGE_CONTROL_1_REG10H, &chrg_ctrl1_reg10.byte, ONE_BYTE);
...
error_type bq25792_start_adc_conversion(void)
{
    ...
    /* Start ADC conversion */
    result = bq25792_write_reg(ADC_CONTROL_REG2EH, &adc_ctrl_reg.byte, ONE_BYTE);
    ...
    result = bq25792_write_reg(ADC_FUNCTION_DISABLE_0_REG2FH, &adc_func_dis0_reg.byte, ONE_BYTE);
    ...
    result = bq25792_write_reg(ADC_FUNCTION_DISABLE_1_REG30H, &adc_func_dis1_reg.byte, ONE_BYTE);
...
/*! \brief  This function provides write functionality to the Battery charger IC BQ25792.
...
error_type bq25792_write_reg(uint8_t reg_addr, uint8_t *data, uint16_t len)
{
    ...
    if ( len <  ( ARR_ELEMENTS( txbuf ) - 1) )
    {
        ...
        do
        {
            ...
            result = hal_i2c_master_write(bq25792_dev_inst.dev_id, bq25792_dev_inst.slave_address, txbuf, len + ONE_BYTE);
...
/*! \brief  This function provides read functionality to the Battery charger IC BQ25792.
...
{   
    ...
    do
    {
        result = hal_i2c_master_writeread(bq25792_dev_inst.dev_id, bq25792_dev_inst.slave_address, &reg_addr, BQ25792_REGISTER_SIZE_IN_BYTES, data, len);
